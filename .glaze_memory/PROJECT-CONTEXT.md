...
- **Providers (14 in domain/types; 14 registered in this worktree):** GitHub (Actions runs), Cloudflare (Pages + Workers deploys), Supabase (latest migration + error-log rollup), Netlify (site deploys), Resend (domain verification + broadcasts), Grafana (configured alerts + opt-in data source health/dashboards/annotations/live query discovery), Heroku (latest release), Sentry (unresolved issues), PagerDuty (incidents), Statuspage (incidents/components), Datadog (monitors), Honeycomb (triggers/SLOs), PostHog (error-tracking issues/exceptions), Better Stack (Telemetry SQL/logs). Each = one encrypted secret + optional non-secret config fields plus provider collection-area metadata.
- **Features:**
  - Connect multiple accounts per provider; credentials stored encrypted (safeStorage).
  - Command Center (`/`) is the first screen, using task-focused ops modules rather than generic overview charts: failed pipelines, running work, providers needing setup, live attention rows, down checks, firing alert-rule/SLO-risk evidence, active incidents, notification suppression from global snooze/maintenance windows/per-rule snoozes, suggested next actions, and scoped direct handoffs into exact Accounts, Pipelines, Provider workspaces, Incidents, Uptime checks, Alert Rules, Insights/SLO risk, Settings suppression controls, Timeline activity context, and Health detail item log/detail context.
  - Pipelines (`/pipelines`) is a first-class triage screen for normalized `run`, `deploy`, `release`, and `migration` rows across GitHub Actions, Cloudflare Pages/Workers, Netlify deploys, Heroku releases, Supabase migrations, and any provider that emits those categories; it includes running/queued, failed, recent, and deploy-before-failure/incident correlation sections, owner/tier/dependency filters, provider/account/service/status/category/actor/branch filters, log/provider-link actions, local incident creation, alert-rule drafts, and CSV export.
  - Health detail (`/dashboard`) replaces the former generic dashboard overview with current action-state controls, failed/warning/pipeline filters, incident/alert handoffs, project-group/account sections, compact group health headers with affected accounts/latest retained event/status stack, Timeline drilldown, filtered CSV export, row-level log/detail, shared investigation drawer actions, and alert-rule draft actions.
  - Apps cockpit (`/apps`) derives first-class service/app health from project groups/accounts, with selectable service detail, filtered CSV export, local service metadata (owner/tier/runbook/dashboard/repo/dependencies/notes), owner/tier/dependency metadata filters, a dependency overview, health contributors, account coverage with edit/refresh remediation actions, related uptime checks, active incidents, signals, metric summaries with provider-open/alert-draft actions where applicable, stale-account state, provider coverage, deep links, responsive active incident/signal rows with provider-open/local-incident/alert-draft actions, and a cross-provider incident timeline with open/investigate/alert-draft row actions, all narrowed by persisted per-tab filters.
  - Provider workspaces (`/providers`) provide read-only provider/account deep dives across all registered providers, with a provider/account/range picker, responsive section navigation, overview stats, Health charts, provider-native Inventory tables, Evidence rows from retained events plus current log-capable rows/signals/incidents/deep links, provider-scoped alert-rule templates, actionable setup rows from provider metadata, Pipelines handoff, refresh, command-palette handoffs, compact export actions, and secret-free CSV/JSON workspace export backed by current snapshots, retained history, and backend-only adapter `buildWorkspace` enrichment rather than mock data. Grafana now contributes actionable workspace tables for configured collection areas, alert rules, dashboard/annotation/log/trace setup, data source health, and discovered Loki/Tempo/Prometheus capabilities while remaining read-only.
  ...
  - Incident center (`/incidents`) consolidates durable local incidents, responsive live signals/incidents, main Dashboard/Apps monitor-row incident creation handoffs, Uptime down-check incident creation handoffs, retained Timeline/dashboard-event incident creation handoffs, Apps live signal/incident handoffs, filtered CSV export, local acknowledge/silence state, service owner/runbook context from local metadata, owner/tier/dependency service filters, an investigation workspace with evidence summaries/links/inline notes that stack instead of squeezing on narrow widths, deterministic investigation hints, copyable follow-up checklists, copyable postmortem drafts from local incident evidence, local incident lifecycle timelines, per-item history timelines with open actions, structured Markdown incident report export, and redacted JSON incident export.
  ...
  - Custom Dashboards (`/dashboards`) lets users create, import, export, and template persisted Recharts dashboards with investigation starter templates, local all-provider monitor-history panels plus live provider query panels for adapters that explicitly expose configured dashboard query capabilities; dashboard definitions can persist non-secret variables for group/provider/account/check/owner/tier/dependency defaults that apply to local panels unless a panel has a narrower explicit scope, dashboard runtime filters can temporarily override those variables, individual panels can override the dashboard range and refresh cadence, secondary dashboard actions live in a compact non-Radix More menu to keep the header responsive without Radix Slot crashes, and stat/log/trace/event results get bounded, dedicated evidence renderers/actions instead of only generic tables; the old dedicated Grafana tab has been removed from navigation.
...
- `renderer/main/command-center-view.tsx` — first-screen compact ops summary using existing renderer data hooks (`monitor:getSnapshot`, monitor settings, retained history events, accounts, checks, dashboards, local incidents, alert rules/states, SLO statuses) to show task-focused modules for failed pipelines/running work/providers needing setup, a primary live attention column, firing alert-rule rows with current values/thresholds, SLO risk with row-level budget/burn-rate/sample evidence, active notification snooze/maintenance-window/per-rule snoozes, suggested next actions, account attention, and active incident queue without adding storage or IPC; rows and buttons write existing one-shot handoff payloads for exact account edit, Pipelines, provider workspaces, uptime check filters, firing alert-rule selection/filtering, rule-snooze review, SLO risk scoped to Insights filters, Settings suppression review, live/local incident drilldowns, retained Timeline context with account-derived group fallback, and Health detail item log/detail context.
- `renderer/main/pipelines-view.tsx` — first-class pipeline triage route at `/pipelines`; derives `PipelineRow` from normalized monitor items with categories `run`, `deploy`, `release`, and `migration`; filters by range/provider/account/service/status/category/actor/branch/owner/tier/dependency/search; shows Running or queued now, Failed recently, Deploys before failures or incidents, and Recent deploys/releases/runs/migrations; row actions open provider links/logs, create local incidents, draft alert rules, and export filtered CSV.
- `renderer/main/provider-workspace-view.tsx` / `main/services/provider-workspace.ts` / `main/handlers/providers.ts` — read-only provider workspace foundation: `providers:listWorkspaceCapabilities/getWorkspaceOverview/runWorkspaceQuery/exportWorkspace` IPC builds provider/account-scoped workspaces from current aggregate snapshots and retained history, loads provider credentials only in the backend when an adapter implements `buildWorkspace`, and returns only typed secret-free series/resources/evidence/alert-template/warning sections; renderer uses the shared responsive route shell with horizontal-to-side section navigation, Overview/Activity/Health/Inventory/Evidence/Alerts/Exports/Setup sections, provider-native table labels, actionable setup rows, Pipelines handoff, refresh, compact export menu, and CSV/JSON export.
- `renderer/main/help-view.tsx` — in-app user documentation route at `/help`; searchable practical docs for first-run setup, provider accounts, configured-only collection areas, portable setup, Command Center, Pipelines, Health detail, Apps, Provider workspaces, Insights/SLOs, Incidents, Timeline, Uptime, Alert Rules, Custom Dashboards as an advanced area, filters/exports, Settings, notification channels, and troubleshooting. The provider setup reference uses `useProviders()` / `providers:list` metadata (`scopeHint`, field labels, required/optional, secret vs non-secret, collection areas) instead of hardcoded credential copies or token values.
...
- `main/handlers/index.ts` — calls `registerProviders()` first, then registers account/channels/checks/dashboards/diagnostics/grafana/history/investigation/local-incident/monitor/provider/rules/service/setup/triage/verification handlers.
...
- `main/handlers/diagnostics.ts` — account diagnostics IPC: list account health, provider retry backoff state, configured collection-area status, provider live-dashboard capability support/counts/names, and run a stored-credential validation/capability load without returning tokens.
- `main/handlers/investigation.ts` / `main/services/investigation-context.ts` — `investigation:getContext` IPC and backend evidence assembler; returns non-secret trigger/account/group/service metadata, current snapshot items, retained 24h events, signals/incidents, metrics, checks, and deep links for an item/event/manual scope without reading provider tokens or making provider API calls.
...
- `main/services/channels-store.ts` — `channels.json` non-secret channel meta; webhook URL stored in token-store under `channel:<id>`. `main/services/dispatch.ts` — `dispatch(event)`/`dispatchTest(id)` POST to enabled Slack/Teams/webhook channels by explicit `event.channelIds` when provided, otherwise by event kind subscription (never throws into the poll cycle); Slack text, Teams Adaptive Card facts, and generic webhook JSON include optional non-secret service context when supplied.
...
- `renderer/main/root-view.tsx` — SplitView + Sidebar nav plus the global shared investigation drawer; route content is wrapped in a `min-w-0 overflow-hidden` guard so child grids/toolbars cannot widen the app pane; `router.tsx` routes (`/`, `/pipelines`, `/dashboard`, `/apps`, `/providers`, `/insights`, `/incidents`, `/timeline`, `/uptime`, `/alerts`, `/dashboards`, `/accounts`, `/help`); secondary data-heavy routes are lazy-loaded so the Command Center route carries less startup code.
- `renderer/main/components/command-palette.tsx` — Cmd/Ctrl-K search/jump surface with shortcut hints for views, actions (refresh/run smoke verification/settings/add account/create dashboard/add uptime check/create local incident/create alert rule/open pipelines), pipeline row jumps, provider workspace jumps, accounts, apps/services, uptime checks, alert rules, dashboards, live incidents/items, local incidents, provider deep links, and service metadata runbook/dashboard/repository links; account, pipeline, provider workspace, app/service, alert-rule, dashboard, dashboard recent-item logs, local-incident, manual incident creation, smoke verification, alert-rule draft, account/check/dashboard creation, and uptime-check results hand off one-shot selection/filter/action payloads so the destination opens the chosen item or workflow instead of only the tab.
- `renderer/main/provider-workspace-view.tsx` (read-only provider/account deep-dive with provider/account/range/search filters, responsive section navigation, overview stats, retained Activity/Health charts, provider-native Inventory tables, Evidence rows, alert-template drafts, actionable setup table, Pipelines handoff, refresh, compact export menu, and CSV/JSON export), `pipelines-view.tsx` (pipeline triage from normalized run/deploy/release/migration rows with running/failed/recent/correlation sections, ownership/context filters, log/provider-link/local-incident/alert-draft actions, and CSV export), `dashboard-view.tsx` (Health detail grouped by account with date/group/provider/account/status/category plus service owner/tier/dependency filters, direct action-state filters, pipeline/incidents/alerts handoffs, history-backed activity, row-level log viewer, row-level shared investigation drawer handoff, one-shot command-palette recent-item log/detail handoff, and reset actions on account/activity filter misses), `apps-view.tsx` (selectable service detail with editable local metadata, owner/tier/dependency filtering, dependency overview, metadata badges on service tiles, health contributors, accounts, provider links, related uptime checks, incidents, signals, metrics, and activity, with distinct no-accounts/no-polling/no-filter-match empty states plus investigation drawer actions and confirmation before clearing service metadata), `insights-view.tsx` (history trends + SLOs + CSV export button + service owner/tier/dependency filters, retained-history no-data vs no-filter-match empty states, SLO filter-miss reset action, plus confirmation before deleting SLOs), `incidents-view.tsx` (local incident lifecycle + live provider sources + Health detail/Pipelines/Apps monitor-row and retained Timeline/dashboard-event incident creation handoffs + service owner/tier/dependency filters + service metadata context + investigation workspace for evidence summaries/provider/runbook links/inline local-incident notes + deterministic investigation hints + copyable local incident follow-up checklist + detail timelines + correlated evidence linking when creating local incidents from live sources or retained events + Markdown report and redacted JSON export actions + local/live filter-miss reset actions + confirmation before deleting local incidents, plus one-shot dashboard drilldown filters), `timeline-view.tsx` (correlation chart/list with service owner/tier/dependency filters, distinct no-history/no-filter-match states, row-level retained-event investigation and incident creation, reset action, plus one-shot dashboard drilldown filters), `uptime-view.tsx` (checks list with status/uptime/latency chart + owner/tier/dependency filters + no-match reset action + add/edit dialog + command-palette add-check handoff + per-check investigation and uptime-down alert draft action + delete confirmation plus one-shot dashboard drilldown filters), `alerts-view.tsx` (rule list with firing/snoozed/channel/service-metadata badges, retained 24h breach/max/suggested-threshold tuning context for sample-backed rules, confirmed quick-apply for differing suggested thresholds, tuning and suggested-threshold columns in CSV export, owner/tier/dependency filters, no-match reset action, one-click rule templates in the add dialog, retained-history threshold suggestions plus recent matching retained-history and check-sample context in the rule dialog, per-rule snooze controls, per-rule channel selection, current-snapshot preview, test delivery, add/edit dialog with dashboard-panel/command-palette/pipeline draft handoff, and delete confirmation), `dashboards-view.tsx` (advanced custom dashboard builder + searchable dashboard selector with match counts and clear action + searchable selected-dashboard panel grid with match counts and clear actions + command-palette template creation handoff + investigation-oriented template picker for Service Health, Incident Investigation, Deploy Correlation, Provider Reliability, Grafana Logs/Traces, Uptime/SLO, and Team/Service Ownership dashboards; panel grid with searchable/filterable Default panels vs Custom query creation modes, capability-filter clear action in the panel editor, editor-side required query/param validation, one-click uptime check latency/uptime defaults that prefill the first available check, unavailable live-capability indicators for saved provider panels, provider x/y mapping controls, provider default params editable without forcing custom-query mode, blank optional provider params removed in editor state, default-query panels reopening in Default mode until the query is customized, panel-level range overrides, explicit local panel scopes for group/account/provider/check/owner/tier/dependency and editable event-type scopes for local event panels, per-panel refresh plus refreshing/stale status text, disabled boundary move controls for first/last panels, delete confirmations for dashboards/panels, at-a-glance panel source/range/scope metadata badges, panel account/provider drilldowns into Accounts, chart-point drilldowns into Timeline/Incidents/Uptime, local chart annotations with clickable retained history events, dashboard duplication, same-dashboard panel duplication, cross-dashboard panel copy, single-dashboard export/import, alert-rule draft actions for supported local panels, local event-row investigation/incident creation, service owner/tier/dependency runtime filters for local panels, a runtime-filter boundary callout when live provider panels are present, bounded, dedicated log/trace evidence renderers, and searchable/sortable/exportable row-level open actions for linked table/log/trace/event rows); all data-heavy views have min-width route wrappers, persisted scoped filters, reset actions, saved filter presets, and shared custom date inputs that cap their oldest selectable timestamp by retained history while allowing the current retained window through now and normalizing older stored custom values when bounds change. `accounts-view.tsx` includes setup checklist with next-action buttons, portable setup import/export with account picker, persisted search/provider/group/enabled/diagnostic/token/dashboard-support filters over accounts and diagnostics, accepts one-shot account/provider/group selection, add-account, and smoke-verification payloads, account diagnostics with collection-area badges and per-account Dashboards actions that open Custom Dashboards scoped to that provider/account, and a live smoke verification runner whose notification-channel delivery tests are opt-in. The old Grafana tab route/sidebar/command-palette entry and renderer view source have been removed; backend Grafana IPC remains for migration/provider support.
- `renderer/main/dashboard-view.tsx` applies retained-history activity group filters in the renderer with account-derived fallback for events missing `groupId`, while live snapshot date filtering remains based on each monitor row's `updatedAt`; it derives Health detail action-state controls and per-group health headers from existing snapshot, retained-history, account, group, provider, and service-metadata hooks without adding backend IPC or storage.
...
- Root navigation supports keyboard shortcuts for the primary operational routes, including Providers at `Cmd/Ctrl+4` and Accounts at `Cmd/Ctrl+0`, plus a sidebar `?`/Help item at `/help`; command-palette navigation rows include Help and matching shortcut hints where applicable.
...
- localStorage: versioned per-tab/filter-surface blobs (`accounts.filters.v1`, `dashboard.filters.v2`, `apps.filters.v1`, `providerWorkspace.filters.v1`, `insights.filters.v2`, `incidents.filters.v1`, `timeline.filters.v1`, `uptime.filters.v1`, `alerts.filters.v1`, `customDashboards.filters.v2`, `notificationChannels.filters.v1`) plus matching `.presets` arrays for saved filter presets, `.presets.default` pinned default preset ids, and transient one-shot navigation/action payloads (`accounts.select.v1`, `accounts.verify.v1`, `alerts.select.v1`, `apps.select.v1`, `dashboard.item.select.v1`, `dashboards.select.v1`, `incidents.select.v1`, `timeline.drilldown.v1`, `incidents.drilldown.v1`, `uptime.drilldown.v1`, `investigation.context.v1`). In-memory: enriched aggregator snapshot (+ last check results); diff-engine last-status map; rules-engine firing-state map.

### IPC channels
- `providers:list` → ProviderInfo[]; `providers:listWorkspaceCapabilities/getWorkspaceOverview/runWorkspaceQuery/exportWorkspace` for read-only provider/account workspace capabilities, retained-history analytics, resources/evidence, alert templates, and secret-free CSV/JSON export; `groups:list` → ProjectGroup[]; `accounts:list` → Account[]; `accounts:add/update/test` (payload `{ provider, label?, creds, groupId?, newGroupName? }`, creds = flat map; secrets inbound only); `accounts:remove`; `accounts:exportSetup/importSetup` for non-secret account/group setup JSON.
- `diagnostics:listAccounts/runAccount` → account health/credential validation metadata plus non-secret collection-area state and dashboard capability support/counts/names after live diagnostics; never returns tokens.
- `investigation:getContext` → non-secret investigation evidence context for an item/event/manual scope, assembled from the current aggregate snapshot, retained 24h history events, service metadata, checks, metrics, incidents/signals, and deep links; does not read provider tokens or call provider APIs.
...
- Adding a provider: create `main/services/providers/<id>.ts` implementing `ProviderDefinition`, register it in `providers/index.ts`, add `<id>` to the `Provider` union in both `types.ts` files, and add an icon/label entry in `renderer/main/components/provider-meta.tsx`. Optional backend-only `buildWorkspace` enrichment can add provider-specific workspace series/resources/evidence/templates without exposing credentials to the renderer. Everything else (dialog, filters, poller) is data-driven.
...
- User-facing functionality changes must keep the in-app Help docs up to date in the same change; provider setup docs should remain data-driven from provider registry metadata and must not duplicate raw secrets or token values.

## Recent History

### 2026-07-08 — Add Pipelines and remove low-signal ops visuals
- **Goal:** Rapidly shift the app from generic visual summaries toward task-focused operations across all providers, with Pipelines and provider-native workspaces as first-class surfaces.
- **What was done:** Added `/pipelines` route/sidebar/command-palette handoffs and `PipelineRow` rendering from normalized `run`, `deploy`, `release`, and `migration` items; replaced Command Center and Health detail generic heatmap/density/trend visuals with direct modules for failed pipelines, running work, provider setup, broken/warning rows, incidents, alerts, and action-state filtering; renamed the detailed Dashboard surface to Health detail; changed Provider workspaces to Overview/Activity/Health/Inventory/Evidence/Setup language with provider-native table labels, setup guidance rows, and Pipelines handoff; added backend-only `buildWorkspace` execution with credential loading confined to `main/services/provider-workspace.ts`; added actionable Grafana workspace tables for collection areas, alert rules, data source health, dashboard/annotation/log/trace setup, and discovered Loki/Tempo/Prometheus capabilities. Updated Help docs, contract checks, and this project context.
- **Key decisions:** This pass stays read-only and reuses normalized snapshot/history rows; provider credentials are used only in backend adapter enrichment and returned workspace content remains typed and secret-free. Custom Dashboards remain advanced/custom rather than the required place to find core provider features.
- **Validation:** `npm run type-check` passes after implementation; full contract/lint/build validation was run after this context update.

### 2026-07-08 — Add triage-first Command Center and Dashboard visuals
- **Goal:** Make Command Center and the detailed Dashboard more useful at a glance by showing where operational risk is concentrated and what changed recently, not just lists and counts.
- **What was done:** Added shared renderer visual primitives for stacked status bars, event-density strips, and heatmap cards; upgraded Command Center with an operations map covering issue mix, affected project groups/providers, and retained 24h event density; upgraded `/dashboard` with retained success/failure and incident/alert trends, current status mix, category breakdown, project-group health heatmap, and per-group triage headers with issue filtering and Timeline handoffs. Updated Help docs and this project context.
- **Key decisions:** This is renderer-only and reuses existing snapshot/history/account/group/provider/service-metadata hooks plus existing one-shot localStorage drilldowns; no storage format, IPC channel, provider API, or mock data was added.
- **Validation:** `npm run test:contracts`, `npm run type-check`, `npm run lint`, and `npm run build` pass. `npm run dev:renderer` starts at `http://localhost:4143/`, but `/` and `/dashboard` return HTTP 404 in renderer-only mode, so a rendered visual check could not be completed there.

### 2026-07-08 — Recover minimum-window route usability
- **Goal:** Make provider workspaces and the major operational tabs usable at the app's 720px minimum window before adding deeper provider-specific data.
- **What was done:** Added shared responsive route primitives (`RouteSurface`, `RouteHeader`, wrapping toolbar, compact More menu, responsive section nav, auto-fit grids, and table-local scroll), rewired `/providers` to use them, moved provider exports into a compact menu plus an explicit Exports section, added real Activity/Analytics/Exports/Setup scroll targets, delayed the side rail until wide desktop, wrapped route content in a root min-width guard, and added `min-w-0` to the main content wrappers for Dashboard, Apps, Insights, Incidents, Timeline, Uptime, Alerts, Dashboards, Accounts, and Help. Added backend-only `buildWorkspace` provider enrichment types for future provider-specific depth without changing renderer IPC.
- **Key decisions:** This pass prioritizes layout recovery and read-only extension points; current provider workspace data still comes from normalized snapshots and retained history, so no new provider tokens are exposed and no mock rows are fabricated.
- **Validation:** `npm run test:contracts`, `npm run type-check`, `npm run lint`, and `npm run build` pass.

### 2026-07-08 — Add read-only provider workspaces
- **Goal:** Start provider parity and UI upgrade work with a framework-first workspace that gives every provider real charts, resources, evidence, exports, and alert templates without write/admin actions.
- **What was done:** Added token-free `provider-workspace` backend aggregation and `providers:listWorkspaceCapabilities/getWorkspaceOverview/runWorkspaceQuery/exportWorkspace` IPC, shared backend/renderer workspace types, `/providers` route/sidebar item/command-palette handoffs, a provider workspace UI with provider/account/range/search controls, retained status/activity/incident-alert Recharts panels, resource/evidence tables, setup reference, alert template drafts, and CSV/JSON export. Updated setup filter allowlists, Help, README, AGENTS, contracts, and this project context.
- **Key decisions:** This first slice is read-only and uses existing normalized snapshots plus retained history for all 14 providers, so no provider tokens are returned and no mock/vendor-admin data is fabricated; deeper provider-specific inventory can plug into the same contract later.
- **Validation:** `npm run test:contracts`, `npm run type-check`, `npm run lint`, and `npm run build` pass.

### 2026-07-07 — Add Teams notification channels
- **Goal:** Add first-class Microsoft Teams notification delivery without requiring Graph, OAuth, or a Teams bot.
- **What was done:** Extended notification channel types, IPC validation, portable setup parsing, Settings channel UI, alert-rule channel labels, smoke-verification copy, Help docs, and source contract checks for Teams. `dispatch.ts` now sends Teams channels a Teams-compatible Adaptive Card payload with event kind, timestamp, URL, and non-secret service context while preserving Slack text payloads and generic webhook JSON.
- **Key decisions:** Teams channel URLs reuse the existing encrypted `channel:<id>` token-store path and remain write-only to the renderer. Portable setup exports/imports Teams channel metadata only and imports channels disabled until users re-enter URLs.
- **Validation:** `npm run test:contracts`, `npm run type-check`, and `npm run lint` pass.

### 2026-07-07 — Add explicit stacked text spacing across operational cards
- **Goal:** Stop adjacent `Text` components from visually concatenating in stacked labels, values, titles, and metadata, including Uptime check cards like `Marketing Site Health CheckGET`.
- **What was done:** Made stacked text in Uptime, Custom Dashboards, Alerts, Apps, Accounts, Incidents, Insights, Timeline, Help, and shared account/investigation dialogs explicitly block-level, and added small flex-column gaps to metric cards where labels and values are vertically grouped. Updated Help and this project context.
- **Key decisions:** This is a renderer-only typography/layout fix; no IPC, storage, provider query, filtering, export, alert, incident, or dashboard-definition behavior changed.
- **Validation:** `npm run type-check`, `npm run test:contracts`, `npm run lint`, and `npm run build` pass. Rendered Browser validation was attempted through `npm run dev:renderer`, but the renderer dev server returned HTTP 404 for `/` and `/uptime`, so the in-app Browser could not render the route.

### 2026-07-07 — Fix custom dashboard trace row overflow
- **Goal:** Stop long Grafana/Tempo trace names and trace IDs from colliding with duration badges and row action buttons in Custom Dashboards trace panels.
- **What was done:** Updated the trace evidence renderer so title, service/trace-id metadata, and timestamp render as bounded block rows inside the text column, while duration badges and action buttons keep fixed space on the right. Updated the in-app Help docs and this project context.
- **Key decisions:** This is a renderer-only layout fix for Custom Dashboards trace results; no dashboard storage, provider query, IPC, export, or investigation behavior changed.
- **Validation:** `npm run type-check`, `npm run test:contracts`, and `npm run lint` pass. Rendered Browser validation was attempted through `localhost`, `[::1]`, and a temporary `127.0.0.1` proxy, but the in-app Browser controller blocked the local URL with `ERR_BLOCKED_BY_CLIENT`.

### 2026-07-06 — Fix Dashboards More menu slot crash
- **Goal:** Stop the Dashboards `More` action menu from crashing when opened.
- **What was done:** Replaced the Radix-backed Dashboards `More` popover with a small local disclosure menu using the existing Glaze `Button` styling, click-away close behavior, Escape close behavior, and the same refresh/edit/duplicate/export/import/templates handlers. The visible toolbar behavior is unchanged, but opening the menu no longer routes through Radix `Popover.Content` slot wrappers that were throwing `Primitive.div failed to slot onto its children`.
- **Key decisions:** This remains a renderer-only UI fix; no dashboard storage, export/import format, IPC, provider, or panel execution behavior changed.
- **Validation:** `npm run type-check` passes after the implementation change. Full static validation was run after updating this project context.

### 2026-07-06 — Fix responsive row overflow and dashboard actions
- **Goal:** Remove visible text collisions in dense Dashboard/Apps/Incidents rows and make the Custom Dashboards toolbar responsive.
- **What was done:** Reworked dashboard run/activity rows, account headers, Apps service/incident/signal/metric/timeline rows, and Incidents live-source/investigation rows so long titles, provider labels, and error messages truncate inside bounded text columns instead of overlapping badges, timestamps, or action buttons. Moved Custom Dashboards secondary actions (refresh/edit/duplicate/export/import/templates) into a compact More menu while keeping search, dashboard picker, filters, and `+ Dashboard` visible. Updated Help and this project context.
- **Key decisions:** This is a renderer-only layout change; no IPC, storage, provider, export, or dashboard definition behavior changed.
- **Validation:** `npm run type-check`, `npm run test:contracts`, `npm run lint`, and `npm run build` pass. Rendered browser validation was attempted with `npm run dev` and `npm run dev:renderer`, but this workspace could not launch the native dev process because `@glaze/core` was not resolvable there, and renderer-only Vite hit the known SDK `sonner` resolution blocker.

### 2026-07-06 — Add investigation-first observability flow
- **Goal:** Make the app feel like an investigation-first observability platform instead of a set of raw provider panels, with advanced collection areas explicit and configured-only.
- **What was done:** Added provider collection-area metadata, account diagnostics/setup UI for collection status, configured-only Grafana data-source health/live query discovery defaults, `investigation:getContext` IPC, a global shared investigation drawer, evidence-first actions from Dashboard/Apps/Timeline/Uptime/Custom Dashboards, dedicated dashboard log/trace renderers, dashboard result presentation metadata, and investigation-oriented dashboard templates including configured Grafana logs/traces. Updated Help, README, AGENTS, contracts, and this project context.
- **Key decisions:** Investigation context uses current snapshot, retained 24h local history, service metadata, checks, metrics, incidents/signals, and deep links only; it does not read tokens or call provider APIs. Saved Grafana observability presets still enable live query capabilities for migrated accounts.
- **Validation:** `npm run type-check`, `npm run test:contracts`, `npm run lint`, and `npm run build` pass.

### 2026-07-06 — Add in-app user Help docs
- **Goal:** Give users a first-class `?` Help area that documents setup and the full app from a user perspective, and make docs updates mandatory for future user-facing changes.
- **What was done:** Added `renderer/main/help-view.tsx` with searchable structured guides for setup, provider accounts, portable setup, Command Center, Dashboard, Apps, Insights/SLOs, Incidents, Timeline, Uptime, Alert Rules, Custom Dashboards, filters/exports, Settings, notification channels, and troubleshooting. Wired `/help` into the router, sidebar, and command palette, and rendered provider setup reference from live `providers:list` metadata. Updated `AGENTS.md` and contract checks so user-facing functionality changes must update in-app Help docs.
- **Key decisions:** Help is renderer-only and adds no backend IPC or storage. Provider setup docs use safe registry metadata and never include raw token values or webhook URLs.
- **Validation:** `npm run test:contracts`, `npm run type-check`, `npm run lint`, and `npm run build` pass.

### 2026-07-04 — Redesign Command Center as compact ops console
- **Goal:** Fix the Command Center's awkward wide-screen layout while preserving the native Glaze look and existing action handoffs.
- **What was done:** Reworked `renderer/main/command-center-view.tsx` into a compact system-health overview band plus a three-column workbench: left rail for next actions/suppression/accounts, center primary live attention queue, and right rail for alert rules, SLO risk, incidents, and retained activity. Added shared local panel/metric helpers and visible hover/focus states for clickable rows without adding backend IPC, storage, custom CSS, fonts, or raw palette colors.
- **Key decisions:** The redesign stays within existing Glaze primitives and semantic tokens, keeps all existing renderer hooks and one-shot destination payloads, and treats this as a layout/hierarchy change only.
- **Validation:** `npm run test:contracts`, `npm run type-check`, `npm run lint`, and `npm run build` pass. `npm run dev:renderer` starts at `http://localhost:4143/`, but browser inspection is blocked by Vite failing to resolve the Glaze SDK component bundle's `sonner` import in renderer-only mode.
